function accelSRP_J2000_m_s2 = computeSRPcartesianPertubation_J2000(cartesianState, A, SRPcoeff, m, p_sr, sunPosition, Re)
...
eclipseAngle = acos(dot(-sunPosition, cartesianState) / (norm(sunPosition) * norm(cartesianState)));
eclipseCondition = norm(cartesianState) * sin(eclipseAngle) < Re;

if not(eclipseCondition)
    SRPfactor = - SRPcoeff * A / m * p_sr / relative_r^3;
