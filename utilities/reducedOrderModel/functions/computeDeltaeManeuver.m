function [maneuverLocations,maneuverVectors] = computeDeltaeManeuver(DeltaROE_control,T_duration, OE)
...
[nuOpt, dv_min] = computeDeltaeDominance(Delta_dex,Delta_dey, e, a);
...
for i = 1:length(combos(:,1))
    ...
    % Check if s = +1 or -1
    G1 = getChernickControlInputMatrix([OE(1:5); M1]);
    G2 = getChernickControlInputMatrix([OE(1:5); M2]);
    G3 = getChernickControlInputMatrix([OE(1:5); M3]);
